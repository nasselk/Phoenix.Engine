import { Vector } from "./vector.js";
export declare const EPSILON = 1e-10;
export declare function clamp(value: number, minimum: number, maximum: number): number;
export declare function wrap(value: number, minimum: number, maximum: number): number;
export declare function sign(value: number): number;
export declare function percentage(value: number, max?: number, round?: boolean): number;
export declare function normalizeArray(array: number[]): number[];
export declare function getBoundingBox(width: number, height: number, angle?: number): Vector;
export declare function distance(x1: number, y1: number, x2: number, y2: number): number;
export declare function distanceSquared(x1: number, y1: number, x2: number, y2: number): number;
