...
> extends EventEmitter<EngineEvents> {
	...
	public readonly world: World<D>;

	public constructor(options: EngineOptions<In, Out, InSchemas, OutSchemas, Actions, R, D>) {
		super();
		...
		this.world = new World<D>({ ...world, capacity: world?.capacity ?? DEFAULT_CAPACITY, role: world?.entities === undefined ? "local" : "mirror" });
	}
	...
	public async init(...promises: Promise<void>[]): Promise<void> {
		...
		// The world first, then the draw: Phase.Render subscribers push world state into the scene
		// graph, and the renderer has to see that frame's positions, not the previous one's.
