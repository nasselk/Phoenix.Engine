...
class HashGrid2D {
    constructor(cellWidth, cellHeight = cellWidth, bounds, removableObjects = true, types) {
        this.bounds = {
            min: new Vector(bounds.min.x, bounds.min.y),
            max: new Vector(bounds.max.x, bounds.max.y),
        };
    ...
    resize(bounds, cellWidth = this.cellWidth, cellHeight = this.cellHeight, restore = true) {
        ...
        if (restore) {
            for (const entity of entities) {
                this.insert(entity, entity.size.x, entity.size.y, entity.type);
            }
        }
        return this;
    }
    ...
    getBounds(object, rangeX = object.size.x, rangeY = object.size.y) {
        ...
        this.boundsBuffer[2] = clamp((localX + rangeX / 2) / this.cellWidth, 0, this.maxKeyX) | 0;
        this.boundsBuffer[3] = clamp((localY + rangeY / 2) / this.cellHeight, 0, this.maxKeyY) | 0;
        return this.boundsBuffer;
    }
    insert(object, rangeX, rangeY, type = object.type) {
        const bounds = this.getBounds(object, rangeX, rangeY);
        const minX = bounds[0];
        const minY = bounds[1];
        const maxX = bounds[2];
        const maxY = bounds[3];
    ...
    update(object, rangeX, rangeY, type = object.type) {
        const bounds = this.getBounds(object, rangeX, rangeY);
        const minX = bounds[0];
        const minY = bounds[1];
        const maxX = bounds[2];
    ...
    query(object, callback, rangeX, rangeY, param1, param2) {
        const bounds = this.getBounds(object, rangeX, rangeY);
        const minX = bounds[0];
        const minY = bounds[1];
        const maxX = bounds[2];
